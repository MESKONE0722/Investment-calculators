...
with tabs[0]:
    ...
    freq = st.selectbox("Compounding Frequency", [1, 4, 12], format_func=lambda x: {1: "Yearly", 4: "Quarterly", 12: "Monthly"}[x])
    if st.button("Calculate Compound Interest"):
        ...
        st.dataframe(df.style.format({"Deposit": "${:,.2f}", "Interest": "${:,.2f}", "Ending Balance": "${:,.2f}"}))

# --- DCA / SIP Tab ---
with tabs[1]:
    st.header("💸 Dollar-Cost Averaging (DCA) / SIP Calculator")
    monthly = st.number_input("Monthly Contribution ($)", min_value=0.0, value=100.0)
    rate2 = st.slider("Expected Annual Return (%)", 0.0, 20.0, 8.0)
    years2 = st.slider("Duration (Years)", 1, 50, 10)
    if st.button("Calculate DCA"):
        df2 = sip_schedule(monthly, rate2, years2)
        st.line_chart(df2.set_index("Month")["Balance"])
        st.dataframe(df2.style.format({"Contributed": "${:,.2f}", "Balance": "${:,.2f}"}))
